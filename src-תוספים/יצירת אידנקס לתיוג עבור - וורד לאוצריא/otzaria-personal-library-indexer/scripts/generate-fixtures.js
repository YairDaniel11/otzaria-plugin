...
fs.mkdirSync(path.join(fixturesDir, 'subject'), { recursive: true });
...
(async () => {
  ...
  // נבנה שני קבצים בשם "sample.pdf" בתת-תיקיות שונות (שורש fixtures ו-fixtures/subject)
  // כדי לבדוק את סכמת מניעת ההתנגשויות בשמות הפלט כשברירת המחדל מרכזת הכל
  // לתיקייה שטוחה אחת (_אינדקס_אישי).
  const {
    PDFDocument,
    StandardFonts,
    PDFName,
  ...
  async function buildOutlinePdf(chapterSuffix) {
    ...
    const item1 = context.obj({
      Title: PDFHexString.fromText(`פרק א${chapterSuffix}`),
      Parent: outlineRef,
      Dest: context.obj([pageRefs[0], PDFName.of('Fit')]),
      Next: item2Ref,
    });
    const item2 = context.obj({
      Title: PDFHexString.fromText(`פרק ב${chapterSuffix}`),
      Parent: outlineRef,
    ...
    const item2a = context.obj({
      Title: PDFHexString.fromText(`פרק ב${chapterSuffix} - תת סעיף מקונן`),
      Parent: item2Ref,
      Dest: context.obj([pageRefs[3], PDFName.of('Fit')]),
    });
    const item3 = context.obj({
      Title: PDFHexString.fromText(`פרק ג${chapterSuffix} - תת סעיף`),
      Parent: outlineRef,
  ...
  const rootPdfBytes = await buildOutlinePdf('');
  fs.writeFileSync(path.join(fixturesDir, 'sample.pdf'), rootPdfBytes);

  // אותו שם בסיס ("sample.pdf") בתת-תיקייה אחרת - בדיקת קידומת מניעת התנגשויות.
  const subjectPdfBytes = await buildOutlinePdf(' (subject)');
  fs.writeFileSync(path.join(fixturesDir, 'subject', 'sample.pdf'), subjectPdfBytes);
