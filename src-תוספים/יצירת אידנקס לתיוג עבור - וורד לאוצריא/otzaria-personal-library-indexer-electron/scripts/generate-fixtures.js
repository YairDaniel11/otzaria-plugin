...
(async () => {
  ...
  // נכתב פעמיים באותו שם בסיס ("sample.pdf") בשתי תת-תיקיות שונות (שורש, ו-sub),
  // כדי לבדוק את מנגנון מניעת ההתנגשות בשמות קובצי הפלט (שני "sample.pdf" שונים
  // חייבים להפיק שני שמות פלט שונים בתיקיית הפלט השטוחה).
  const {
    PDFDocument,
    StandardFonts,
    PDFName,
  ...
  async function makeOutlinedPdf(outPath) {
  ...
  await makeOutlinedPdf(path.join(fixturesDir, 'sample.pdf'));
  await makeOutlinedPdf(path.join(fixturesDir, 'sub', 'sample.pdf'));
