...
**גרסה: 2.0 | עבור מפתחים ו-AI כאחד**

---

## תוכן עניינים

1. [מבוא ועקרונות יסוד](#1-מבוא-ועקרונות-יסוד)
2. [מבנה התוסף וקובץ manifest.json](#2-מבנה-התוסף-וקובץ-manifestjson)
3. [הרשאות — רשימה מלאה וכללים](#3-הרשאות--רשימה-מלאה-וכללים)
4. [עיצוב — Material Design 3 ושפת העיצוב של אוצריא](#4-עיצוב--material-design-3-ושפת-העיצוב-של-אוצריא)
5. [מבנה HTML — שלד מלא](#5-מבנה-html--שלד-מלא)
6. [כללי UX וממשק משתמש אחיד](#6-כללי-ux-וממשק-משתמש-אחיד)
7. [גלילה, פריסה ותמיכה במובייל](#7-גלילה-פריסה-ותמיכה-במובייל)
8. [API — כל הקריאות הזמינות](#8-api--כל-הקריאות-הזמינות)
9. [אירועים (Events)](#9-אירועים-events)
10. [אחסון נתונים](#10-אחסון-נתונים)
11. [בטיחות קוד — XSS, Timezone ועוד](#11-בטיחות-קוד--xss-timezone-ועוד)
12. [ספריות חיצוניות, גופנים ורשת](#12-ספריות-חיצוניות-גופנים-ורשת)
13. [אייקונים — FluentUI System Icons](#13-אייקונים--fluentui-system-icons)
14. [הגדרות (Settings Panel)](#14-הגדרות-settings-panel)
15. [פרסום בחנות התוספים](#15-פרסום-בחנות-התוספים)
16. [שגיאות נפוצות — מה לא לעשות](#16-שגיאות-נפוצות--מה-לא-לעשות)
...
1. **[חובה] אין גישה חופשית לרשת** — כל URL חיצוני חייב להיות ברשימת ה-allowlist של אוצריא (פרט 12).
2. **[חובה] כל הצבעים מגיעים מה-API** — אין לקודד צבעים ישירות בקוד.
3. **[חובה] עיצוב אחיד לכל התוספים** — שפת עיצוב Material Design 3, בדיוק כמו אוצריא עצמה.
4. **[חובה] הרשאות מינימליות בלבד** — בקשו רק מה שאתם אכן משתמשים בו.
5. **[חובה] גלילה — חובה להגדיר באופן מפורש** — WebView לא גולל אוטומטית.
...
| שדה | תיאור | דוגמה |
|-----|--------|-------|
| `id` | [חובה] מזהה ייחודי בפורמט reverse-domain | `"com.yossi.mytool"` |
| `name` | [חובה] שם התוסף כפי שיוצג | `"מעקב לימוד"` |
| `version` | [חובה] SemVer | `"1.0.0"` |
| `description` | [חובה] תיאור קצר | `"מעקב יומי אחר לימוד..."` |
| `author` | [חובה] שמכם | `"יוסי לוי"` |
| `type` | [חובה] תמיד `"webapp"` | `"webapp"` |
| `category` | [חובה] קטגוריה | `"Utilities"` |
| `stability` | [חובה] `"beta"` או `"stable"` | `"beta"` |
| `minAppVersion` | [חובה] גרסה מינימלית נדרשת של אוצריא | `"0.9.91"` |
| `entrypoint` | [חובה] קובץ HTML ראשי | `"index.html"` |

**הערה לגבי API Versioning ו-`minAppVersion`:**
אם התוסף שלכם משתמש בפונקציות API חדשות שנוספו בעדכונים האחרונים של אוצריא, עליכם לוודא ששדה ה-`minAppVersion` מעודכן לגרסה שבה הפונקציה הוצגה. אחרת, התוסף יקרוס אצל משתמשים עם גרסאות ישנות.
...
כל הרשאה שבקוד שלכם אין קריאה ל-API המתאים — **הסירו אותה**. הרשאות מיותרות מעוררות חשד, פוגעות באמון המשתמשים, ועלולות לגרום לדחיית התוסף מהחנות.
...
### [חובה] הרשאות נפוצות שגויות ומבלבלות — אל תכניסו אותן!

הרבה מפתחים מתבלבלים ומבקשים הרשאות לא נכונות. ודאו שאינכם עושים את הטעויות הבאות:

❌ `app.info.read` לצורך קבלת אימייל — **הנכון:** `app.user_email.read`.
❌ `navigation.write` כדי להאזין לאירועי מעבר מסך — **הנכון:** `events.subscribe:navigation.changed` (הראשון משמש ל**ביצוע** מעבר, השני ל**האזנה**).
❌ `settings.read` כדי להאזין לשינוי הגדרות — **הנכון:** `events.subscribe:settings.changed` (הראשון משמש לקריאה אקטיבית של הגדרה).

> שימו לב: הרשאות `calendar.read`, `settings.read`, `navigation.write` וכו' הן הרשאות **חוקיות וקיימות** אם אתם אכן קוראים אקטיבית למתודות ה-API המתאימות (כמו `calendar.getSelectedDate`, `settings.get`, `navigation.goTo`). הטעות היא להשתמש בהן במקום ב-`events.subscribe:*` כאשר הכוונה היא רק להאזין לאירועים.
...
```css
/* ❌ [אסור לחלוטין] צבעים קשיחים ישברו מצב כהה/בהיר וערכות נושא! */
color: #6750A4;
background: white;
...
### פונקציית applyTheme — [חובה] חובה להפעיל!
...
// ✅ פונקציה מוגנת שמתמודדת עם Hex מלא, חלקי, שגוי או Null.
function hexToRgba(hex, alpha) {
  if (!hex) return `rgba(0, 0, 0, ${alpha})`;
  
  hex = hex.replace(/^#/, '');
  
  // טיפול בקיצור hex (למשל F00)
  if (hex.length === 3) {
...
```css
.btn-secondary {
  /* תמיכה לאחור במידה ו-color-mix לא נתמך (למשל WebView ישן) */
  background: var(--color-surface);
...
.interactive:hover {
  /* גיבוי למערכות שלא תומכות ב-color-mix */
  background: var(--color-primary-subtle);
...
העתיקו והשתמשו בשלד הבא. הוא כולל את ההגנות הנדרשות, ה-CSS Variables, פונקציות ה-Sanitization, הטיפול בתאריכים, פאנל ההגדרות והגלילה התקינה.
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* כותרת וכפתור סגירה לפאנל ההגדרות */
    .settings-header {
      display: flex;
...
<body>
  ...
  <aside class="overlay-panel" id="settingsPanel">
    <div class="settings-header">
      <h2>הגדרות</h2>
      <button class="close-btn" id="closeSettingsBtn" aria-label="סגור">
        <!-- אייקון סגירה מ-FluentUI (dismiss_24_regular) -->
  ...
  <script>
    ...
    // הגנה מ-XSS (מנקה טקסט שעלול להכיל תגיות זדוניות)
    function esc(s) {
      if (s == null) return '';
    ...
    function applyTheme(theme) {
      ...
      document.body.classList.toggle('dark-mode', theme.mode === 'dark');
      if (theme.mode === 'dark') {
          r.style.setProperty('--color-scrim', 'rgba(0, 0, 0, 0.5)');
      } else {
          r.style.setProperty('--color-scrim', 'rgba(0, 0, 0, 0.3)');
      }
    ...
    Otzaria.on('plugin.boot', async (payload) => {
      // חובה! אם התוסף מריץ פעולות רקע (דרך app.run_on_startup), 
      // עלינו לחסום הפעלת UI אם אנחנו ב-background.
      if (payload.app && payload.app.runMode === 'background') {
        // ... קוד הרקע שלך מגיע לכאן (ללא UI)
        return; 
      }

      applyTheme(payload.theme);
      // ... אתחול ה-UI והקוד שלך כאן
    });
...
אלה כללים **[חובה] מחייבים** לכל התוספים בחנות אוצריא — לא המלצות:
...
✅ [מותר] הגדרות שמשפיעות על התנהגות התוסף
✅ [מותר] בחירות ברירת מחדל
✅ [מותר] כפתור "אודות" עם שם המפתח ופרטי יצירת קשר
❌ [אסור] לוגיקת תוסף ראשית — ההגדרות זה רק הגדרות!
...
```html
<!-- [חובה] תמיד הוסיפו aria-label לכפתורים ללא טקסט -->
<button aria-label="הגדרות">...</button>
<button aria-label="סגור">...</button>

<!-- [חובה] תמיד הוסיפו alt לתמונות -->
<img src="icon.png" alt="אייקון מועדפים" />

<!-- [חובה] שדות קלט תמיד עם label -->
...
/* ── [אופציונלי] אם יש container גלילה מותאם ──────────────── */
.scroll-container {
  height: 100%;
...
### פריסה רספונסיבית — [מומלץ] תמיכה בגדלים שונים
...
```javascript
// [מומלץ] תמיד השתמשו ב-try/catch בעבודה מול ה-API
try {
...
// ⚠️ ערכת צבעים (בדרך כלל לא צריך לקרוא ידנית — מגיע אוטומטית ב-plugin.boot וב-theme.changed)
...
// עץ ספרייה מלא
// הבהרה למפתחים: פונקציה זו חדשה ועשויה שלא להופיע בממשקי ה-TS הישנים.
...
// ⚠️ [זהירות] התוצאה תהיה null אם המשתמש לא סימן שום טקסט או אם הוא במסך אחר!
if (sel !== null) {
  console.log(`הטקסט ${sel.text} נבחר במיקום: ${sel.currentRef}`);
} else {
  console.log('לא נבחר שום טקסט.');
}
```
...
```javascript
// ⚠️ [זהירות] התאריך שמוחזר הוא ISO 8601 ב-UTC!
const { data: date } = await Otzaria.call('calendar.getSelectedDate');
// "2026-04-08T00:00:00.000Z"

// [חובה] כדי למנוע באג חצות, חלצו את התאריך המקומי בלבד:
const safeDate = localDateStr(new Date(date));
...
**טיפ:** שמרו אובייקט אחד עם כל ההגדרות (אם הן שייכות לאותו נושא), לא מפתח לכל הגדרה.
```javascript
...
// ❌ פחות יעיל ויגרום ליותר קריאות רשת מול ה-RPC
...
// התראה מתוזמנת — [חובה] scheduledTime חייב להיות ISO 8601 UTC!
...
await Otzaria.call('publishedData.upsert', {
  type: 'calendar.event',
  // ההשפעה של scope:
  // 'global'         — יהיה זמין ויוצג תמיד (בכל מקום באפליקציה)
  // 'workspace:<id>' — יוצג רק בסביבת העבודה הספציפית
  // 'book:<bookId>'  — יוצג רק בהקשר לספר מסוים (הערות, קישורים)
  scope: 'global',
  key: 'myplugin:event1',
  payload: {
    title: 'שקיעה',
...
```javascript
// רשימת מקורות זמינים
const { data: sources } = await Otzaria.call('database.listSources');

// סכמה של מקור
const { data: schema } = await Otzaria.call('database.describeSource', { sourceId: 'talmud_synopsis' });

// 🔹 שאילתה דקלרטיבית (כולל שימוש ב-JOIN) 🔹
const { data } = await Otzaria.call('database.query', {
  sourceId: 'talmud_synopsis',
  from: { table: 'tractates', alias: 't' },
  select: [
    { expr: 't.name', as: 'name' },
    { expr: 'p.name', as: 'page' } // מה-JOIN
  ],
  joins: [
    {
...
ריצת רקע מאפשרת לתוסף לקום ולפעול גם ללא פתיחת הכרטיסייה שלו. האפליקציה פותחת למעשה WebView מוסתר שמבצע את ה-`plugin.boot`.

```javascript
Otzaria.on('plugin.boot', async (payload) => {
  // ⚠️ [חובה] ללא בדיקת ה-runMode, הקוד שלך ירוץ פעמיים!
  // פעם אחת מה-WebView שברקע, ופעם שנייה כשהמשתמש פותח את כרטיסיית התוסף.
  
  if (payload.app && payload.app.runMode === 'background') {
    // רץ בעת עליית אוצריא, ללא ממשק נראה
    // השתמשו בזה לתזמון התראות, טעינת נתונים ברקע וכו'
    console.log("אני רץ ברקע ללא UI!");
    return; // עצור כאן כדי לא לרנדר HTML
  }
  
  // אחרת: runMode === 'foreground' — המשתמש פתח ממשק גרפי
  console.log("אני בחזית, בוא נציג UI.");
});
```
...
### ההבדל בין plugin.boot ל-plugin.ready
- **`plugin.boot`**: מופעל כשה-WebView מסיים להיטען, מיד לאחר שהזרקת הסביבה (`Otzaria`) הושלמה. שימושי לקבלת משתנים ראשוניים וחיבור ל-APIs. במצב זה, ה-WebView עלול להיות מוסתר או ברקע.
- **`plugin.ready`**: מופעל כשהתוסף למעשה עבר לקדמת המסך והמשתמש מביט בו בפיזית בטאב שלו (עבר מחזור חיים של הצגה).
...
Otzaria.on('plugin.ready', ()        => { /* לאחר boot והצגה */ });
...
### מגבלות האחסון
- מערכת האחסון בנויה על גבי מסד SQLite. 
- הקריאות רצות ב-RPC ולכן מוגבלות ל-30 שניות. 
- **[מומלץ]** אין לאחסן אובייקטי JSON בנפח של מגה-בייטים רבים במפתח בודד, הדבר יכול לגרור שגיאות timeout בעת הניסיון לשלוף ולפרוק אותם ב-JS. כדאי לפצל נתונים כבדים מאוד אם ישנם למספר מפתחות.
...
// ✅ [נכון] זה בסדר להשתמש ב-toISOString() עבור חותמת זמן לשרת או תזכורות:
...
// ✅ [פתרון 1]: פונקציית esc בטוחה המטפלת גם ב-Null.
function esc(s) {
  if (s == null) return '';
  const d = document.createElement('div');
  d.textContent = String(s);
...
// ✅ [פתרון 2]: textContent ישירות (הכי בטוח)
const div = document.createElement('div');
div.textContent = fileName;
card.appendChild(div);
```

**[חובה] כלל:** כל ערך שלא כתבתם אתם בקוד — `esc()` לפני `innerHTML`.

### טיפול בשגיאות API

```javascript
// [חובה] תמיד בדקו success לפני שימוש ב-data
...
אתם חייבים להעתיק את תג ה-`path` ישירות מקוד המקור של מאגר FluentUI.

```html
<!-- דוגמה: אייקון סגירה (dismiss_24_regular) -->
<button class="settings-btn" aria-label="סגור">
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
    <path d="M6.293 6.293a1 1 0 0 1 1.414 0L12 10.586l4.293-4.293a1 1 0 1 1 1.414 1.414L13.414 12l4.293 4.293a1 1 0 0 1-1.414 1.414L12 13.414l-4.293 4.293a1 1 0 0 1-1.414-1.414L10.586 12 6.293 7.707a1 1 0 0 1 0-1.414z"/>
  </svg>
</button>
...
פאנל ההגדרות צריך להיפתח מהצד השמאל.

### עיצוב מלא (כולל הגנות חזותיות)
...
/* הגבלת הרוחב כדי לא לתפוס 100% מהמסך בניידים */
.overlay-panel {
  width: min(320px, calc(100vw - 20px));
}
```

### שמירת הגדרות חכמה בעזרת Cache

כדי לחסוך בקריאות ל-RPC על כל שינוי הגדרה:

```javascript
// שמירת הגדרות בזיכרון מקומי
let _cachedSettings = null;

async function loadSettings() {
  if (_cachedSettings !== null) return _cachedSettings;
  const { data } = await Otzaria.call('storage.get', { key: 'settings' });
  _cachedSettings = data ?? { showDates: true, itemCount: 25 };
  return _cachedSettings;
}

// עדכון ושמירה אוטומטית
...
// סגירת פאנל בלחיצה על מקש Escape
document.addEventListener('keydown', (e) => {
  const settingsPanel = document.getElementById('settingsPanel');
  if (e.key === 'Escape' && settingsPanel.classList.contains('open')) {
    settingsPanel.classList.remove('open');
...
- [ ] [חובה] כל שדות `manifest.json` קיימים (id, name, version, description, author, type, category, stability, minAppVersion, entrypoint)
- [ ] [חובה] כל הרשאות ב-manifest משמשות בפועל בקוד
- [ ] [חובה] אין הרשאות שאינן קיימות או הרשאות בלבול (ראה רשימה בפרט 3)
- [ ] [חובה] גלילה מוגדרת (`overflow-y: auto` על html/body)
- [ ] [חובה] ה-HTML מתחיל עם `<html lang="he" dir="rtl">`
- [ ] [חובה] `applyTheme` מחובר ל-`plugin.boot` ו-`theme.changed`
- [ ] [חובה] אין צבעים קשיחים בקוד (ללא `#hex`, `rgb(...)`, `white`, `black`)
- [ ] [חובה] שם מפתח אינו בכותרת (רק בחלון אודות)
- [ ] [חובה] כפתור הגדרות בצד שמאל פותח overlay משמאל, שניתן לסגור גם עם Escape.
- [ ] [חובה] כל ספריות חיצוניות ארוזות לוקלית (לא CDN)
- [ ] [חובה] אין שימוש ב-`innerHTML` עם ערכים לא-esc'ים
- [ ] [חובה] אין שימוש ב-`toISOString()` לתאריכים מקומיים
...
החלק הזה מרכז את דפוסי הקוד השגויים הנפוצים ביותר שנמצאו על ידי מערכת הבדיקה לתוספים:

### 1. שגיאות בהרשאות (manifest.json)

```json
// ❌ [שגוי] הרשאות בלבול ופונקציות לא חוקיות
"permissions": [
  "app.info.read",          // ❌ שגוי אם רק רציתם לבקש מייל. הנכון הוא "app.user_email.read"
  "settings.read",          // ❌ שגוי אם רציתם רק להאזין לאירוע settings.changed, נחוץ רק בשביל settings.get אקטיבי!
  "navigation.write"        // ❌ שגוי אם רציתם להאזין ל-navigation.changed, נחוץ רק לביצוע navigation.goTo אקטיבי.
]
```

### 2. ריצת רקע ללא בדיקת `runMode`

תוספים המשתמשים ב-`app.run_on_startup` מחויבים לבדוק את מחזור החיים שלהם (ראו פרק 8).
```javascript
// ❌ [שגוי] אם התוסף מקבל run_on_startup, קוד זה יופעל פעמיים!
Otzaria.on('plugin.boot', async (payload) => {
  renderMyUI(); // יקרוס או יאט ברקע, ויוכפל בחזית.
});
...
### 3. חוסר בדיקת `null` בעת שאיבת סימון משתמש

```javascript
// ❌ [שגוי]
const { data } = await Otzaria.call('reader.getSelection');
console.log(data.text); // יגרום לשגיאת Uncaught TypeError אם אין טקסט מסומן!
...
### 4. באג חצות: שימוש שגוי ב-ISO 8601 לחישוב תאריך נוכחי

```javascript
// ❌ [שגוי] שימוש ב-ISO להחזרת תאריך מקומי
const { data: date } = await Otzaria.call('calendar.getSelectedDate');
const currentStr = new Date(date).toISOString().slice(0, 10); 
// יציג את התאריך של יום אתמול/מחר בשעות הלילה המאוחרות בגלל הפרשי UTC!

// ✅ [נכון] השתמשו תמיד בפונקציה localDateStr שמייצרת מחרוזת בטוחה:
const currentStr = localDateStr(new Date(date));
```

### 5. שגיאות עיצוב ו-UI פנימי
...
// ✅ [נכון — מוגן]
element.innerHTML = `<div>${esc(apiData.name)}</div>`;
element.textContent = storageData; // הדרך הבטוחה ביותר
```
